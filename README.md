...
右键菜单 - 「📁 资源替换」可打开资源管理窗口，将内置的 GIF 动画、音效、卡片图、画作图替换为自定义文件。替换后即时生效，设置自动保存。

### 替换规则

| 资源类别 | 数量 | 允许后缀 | 命名约束 | 内容校验 |
|---------|------|---------|---------|---------|
| GIF 动画 (idle/click/drag/sleep/draw/result) | 6 | .gif | 完全匹配原名 | 加载 GIF 帧 |
| 音效 (启动/抽卡/画画/结果/重置/闹钟/时钟) | 7 | .wav | 完全匹配原名 | - |
| 卡片 (card_1 ~ card_55) | 55 | .png | card_ + 数字 1~55 | 加载 PNG 像素 |
| 画作 (drawing_1 ~ drawing_15) | 15 | .png | drawing_ + 数字 1~15 | 加载 PNG 像素 |
| 作者图 (author) | 1 | .png | 完全匹配原名 | 加载 PNG 像素 |

### 校验流程

1. **后缀校验** - 扩展名是否符合目标类型
2. **命名校验** - 文件名是否符合预期格式（命名资源精确匹配，编号资源正则匹配数字范围）
3. **内容校验** - GIF 尝试加载首帧、PNG 尝试加载像素数据

三项均通过后写入配置。**重置**按钮可恢复为内置资源。

### 配置存储

替换路径保存在 QSettings override/ 分组中（如 override/gif/idle -> C:/my/idle.gif）。启动时自动校验文件是否存在，已删除的替换自动回退到内置资源。
