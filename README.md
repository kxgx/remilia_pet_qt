...
- **手柄键位显示**：程序绘制的手柄图 + 文字气泡，按下按键即时高亮（A/B/X/Y、方向键、肩键/扳机、摇杆方向、L3/R3 等），与键盘气泡并排显示；2 秒无输入自动隐藏，手柄断开立即隐藏
- **音乐信息显示**：独立窗口显示系统当前播放媒体的**标题 / 歌手 / 专辑 / 专辑封面 / 播放状态 / 时间与进度条**（Windows 系统媒体控制 SMTC，兼容浏览器、网易云、Spotify、Windows Media Player 等接入 SMTC 的播放器）；无播放内容时自动隐藏
