...
void ResourceWindow::updateScaleAndPosition(float scale)
{
    ...
    for (int i = 0; i < m_resList->count(); i++)
    {
        ...
        if (!row || !row->layout())
            continue; // category header (scales via list font)
        ...
        if (auto *rb = qobject_cast<QPushButton *>(rowLay->itemAt(2)->widget()))
            rb->setFixedWidth(rbW);
        if (auto *ob = qobject_cast<QPushButton *>(rowLay->itemAt(3)->widget()))
            ob->setFixedWidth(odW);
        if (auto *db = qobject_cast<QPushButton *>(rowLay->itemAt(4)->widget()))
            db->setFixedWidth(odW);
    }
    if (m_hintLabel)
...
void ResourceWindow::refreshAllRows()
{
    for (int i = 0; i < m_resList->count(); i++)
    {
        ...
        QString key = row->property("resourceKey").toString();
        if (key.isEmpty())
            continue; // category header rows have no widget
        QLayout *rowLay = row->layout();
        if (!rowLay)
            continue;
...
void ResourceWindow::initUi()
{
    ...
    topBar->addWidget(m_replaceAllBtn);
    topBar->addWidget(m_openAllBtn);
    topBar->addStretch();
    topBar->addWidget(m_closeBtn);

    m_hintLabel = new QLabel(QString::fromUtf8("\u70b9\u201c\u66ff\u6362\u201d\u9009\u6587\u4ef6\uff0c\u6216\u201c\u5168\u90e8\u66ff\u6362\u201d\u6279\u91cf\u66ff\u6362"), this);
    m_hintLabel->setStyleSheet("color:#888;font-size:10px;padding:2px 4px;");
    ...
    for (int i = 56; i <= 999; i++)
    {
        ...
        if (!QFile::exists(m_pet->m_resourceDir + "cards/card_" + QString::number(i) + ".png"))
            break;
    ...
    for (int i = 16; i <= 999; i++)
    {
        ...
        if (!QFile::exists(m_pet->m_resourceDir + "drawing/drawing_" + QString::number(i) + ".png"))
            break;
    ...
    for (const auto &df : defs)
    {
        if (df.category != curCat)
        {
            ...
            ci->setForeground(QColor("#FF8DA1"));
        ...
        nl->setStyleSheet("color:#fff;font-weight:bold;");
        nl->setFixedWidth(qMax(80, (int)(150 * m_scale)));
        bool over = m_pet->m_resourceOverrides.contains(df.key);
        QLabel *sl = new QLabel(over ? QString::fromUtf8("\u5df2\u66ff\u6362") : QString::fromUtf8("\u9ed8\u8ba4"));
        sl->setStyleSheet(over ? "color:#5f5;" : "color:#888;");
        sl->setFixedWidth(qMax(35, (int)(48 * m_scale)));
        sl->setAlignment(Qt::AlignCenter);

        // Replace button
        QPushButton *rb = new QPushButton(QString::fromUtf8("\u66ff\u6362"));
        rb->setCursor(Qt::PointingHandCursor);
        rb->setFixedWidth(qMax(30, (int)(40 * m_scale)));
        QString k = df.key;
        QString sd = df.subDir;
        QString ext = df.ext;
        QString bn = df.key.section('/', 1);
        ...
        ob->setFixedWidth(qMax(28, (int)(38 * m_scale)));
        connect(ob, &QPushButton::clicked, this, [this, sd]()
                { QString p = m_pet->m_resourceDir + sd; QDir().mkpath(p); openDirInFM(p, [this] { m_pet->scheduleResourceReload(); }); });

        // Delete button
        QPushButton *db = new QPushButton(QString::fromUtf8("\u5220\u9664"));
        db->setCursor(Qt::PointingHandCursor);
        db->setEnabled(over);
        db->setFixedWidth(qMax(28, (int)(38 * m_scale)));
        ...
        m_resList->setItemWidget(it, row);
    }
...
void ResourceWindow::onReplaceAll()
{
    ...
    for (int i = 0; i < m_resList->count(); i++)
    {
        ...
        if (key.isEmpty() || sd.isEmpty() || ext.isEmpty())
            continue;
    ...
    if (count > 0)
        m_pet->scheduleResourceReload(); // re-apply once after the whole batch
...
void ResourceWindow::onOpenAll()
{
    ...
    for (const QString &sd : subDirs)
        d.mkpath(sd);
    openDirInFM(dir, [this]
                { m_pet->scheduleResourceReload(); });
}

void ResourceWindow::closeResourceWindow()
{
