...
class DropHighlightTree : public QTreeView
{
    ...
    void setDropTarget(const QModelIndex &idx)
    {
        ...
        m_dropTarget = idx;
        if (old.isValid())
            viewport()->update(visualRect(old));
        if (m_dropTarget.isValid())
            viewport()->update(visualRect(m_dropTarget));
    }

    void clearDropTarget()
    {
        setDropTarget(QModelIndex());
    }
...
InAppFileDialog::InAppFileDialog(const QString &dirPath, QWidget *parent)
    : QDialog(parent, Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint | Qt::Tool), m_homeDir(dirPath)
{
...
void InAppFileDialog::setupUi(const QString &dirPath)
{
    ...
    connect(m_model, &QFileSystemModel::rowsInserted, this, [this]()
            { updateStatus(); });
    connect(m_model, &QFileSystemModel::rowsRemoved, this, [this]()
            { updateStatus(); });
...
void InAppFileDialog::dropEvent(QDropEvent *event)
{
    ...
    for (const QUrl &url : urls)
    {
        ...
        if (fi.canonicalFilePath() == QFileInfo(dst).canonicalFilePath())
            continue;
        if (QFile::exists(dst))
        {
...
void InAppFileDialog::navigateTo(const QString &path)
{
    ...
    if (!cur.isEmpty() && QFileInfo(cur).canonicalFilePath() == fi.canonicalFilePath())
        return;
...
bool InAppFileDialog::copyPath(const QString &src, const QString &dst)
{
    ...
    while (it.hasNext())
    {
        ...
        {
            if (!QFile::copy(fi.absoluteFilePath(), tgt))
                return false;
        }
    }
    return true;
